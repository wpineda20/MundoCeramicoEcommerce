      ...
      <v-card class="p-3 mt-3">
        <v-container>
          <v-row>
            ...
            <v-col cols="12" sm="12" md="3" lg="3" xl="3" class="d-flex align-items-center"
              ...
              <!-- <base-button type="primary" title="Agregar" disabled @click="addRecord()" /> -->
            </v-col>
        ...
        <v-data-table-server :headers="headers" :items-length="total" :items="records" :loading="loading" item-title="id"
          ...
          <template v-slot:[`item.actions`]="{ item }">
            <!-- <v-icon size="20" disabled class="mr-2" @click="editItem(item.raw)" icon="mdi-pencil" /> -->
            <!-- <v-icon size="20" disabled class="mr-2" @click="deleteItem(item.raw)" icon="mdi-delete" /> -->
          </template>
          ...
          <v-container>
            ...
            <v-row class="pt-3">
              ...
              <v-col cols="12" sm="12" md="12">
                <base-input label="Estado" v-model="v$.editedItem.name.$model" :rules="v$.editedItem.name" />
              </v-col>
              <!-- name -->
            </v-row>
            <!-- Form -->
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`state/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post(`/state`, this.editedItem);
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get(`/state`, {
